...
def test_send_request_when_server_response_false(mocker):
    ...
    assert (
        str(exec_info.value)
        == "server response that it's not success, query: http://test.test"
    )
